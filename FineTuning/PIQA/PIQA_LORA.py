...
def preprocess_data(examples):
    ...
    question_headers = examples["goal"]
    second_sentences = []

    for i, header in enumerate(question_headers):
        repeated_header = [header] * 2
        combined_sentences = [f"{h} {end}" for h, end in zip(repeated_header, [examples['sol1'], examples['sol2']])]
    ...
    tokenized_examples['label'] = [int(answer) for answer in examples['label']] * 2
