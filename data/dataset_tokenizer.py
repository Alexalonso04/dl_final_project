...
# use name="sample-10BT" to use the 10BT sample of fineweb dataset
# use multiple CPU processes to speed up the download
raw_dataset = load_dataset("HuggingFaceFW/fineweb", name="sample-10BT", num_proc=cpu_count() - 4)
...
def save_data(token_array: np.ndarray, shard_index: int):
    ...
    and memoryt
    """
    shard_split = "val" if shard_idx == 0 else "train"
...
with Pool(max(1, cpu_count()//2)) as pool:
    ...
    for tokens in pool.imap(tokenizer, raw_dataset, chunksize=16):
